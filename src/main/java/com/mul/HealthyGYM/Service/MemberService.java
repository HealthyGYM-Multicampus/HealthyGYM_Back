...
public class MemberService {
	...
	public void authCodeEmail(String email, String mailKey) throws Exception {
		...
		String mailContent = "<div style='text-align:center;'>" + 
                ...
                "<p style='font-size:14px; color:#999;'>건강해ZYM | 서울특별시 강남구 테헤란로 1234</p>" +
            "</div>";
	...
	// 비밀번호 재설정 이메일 발송
	public void pwdResetEmail(MemberDto dto, String mailKey) throws Exception {
		...
		String mailContent = "<div style='text-align:center;'>" +
                ...
                "<a href='http://localhost:9100/password/update?email=" + dto.getEmail() + "&mail_key=" + mailKey + "' style='display:inline-block; background-color:#FF4136; color:#fff; font-size:16px; text-align:center; padding:12px 20px; border-radius:5px; text-decoration:none; margin-bottom:30px;'>비밀번호 재설정</a>" +
                "<p style='font-size:14px; color:#999; margin-top:30px;'>본 이메일은 발신 전용입니다. 문의 사항은 고객센터를 이용해주세요.</p>" +
                "<p style='font-size:14px; color:#999;'>건강해ZYM | 서울특별시 강남구 테헤란로 1234</p>" +
                "</div>";		
		...
		// 인증을 위한 이메일 발송	
		MailHandler mailHandler = new MailHandler(mailSender);
		mailHandler.setSubject("[건강해ZYM] " + dto.getNickname() + "님 비밀번호 재설정 이메일 입니다."); // 메일제목
		mailHandler.setText(mailContent);
		mailHandler.setFrom("healthyzym@gmail.com", "건강해ZYM");
